...
def rating_book():
    ...
    if 'loggedin' in session and session['role']=='operator':
        if(request.method == "POST" and form.validate_on_submit()):
           ...
           try:
               query=f"SELECT  AVG(r.grade) AS avg_grade  FROM Review r INNER JOIN Books b ON b.ISBN = r.Books_ISBN INNER JOIN Categories_Books cb ON cb.Books_ISBN = b.ISBN INNER JOIN Schools s ON s.id_school = b.Schools_id_school  WHERE s.Operators_id_operators = '{session['id']}' AND r.approved=1 AND CASE WHEN ('{id_user}' != '' AND '{category}' != '' ) THEN (r.Users_id_user = '{id_user}' AND UCASE(cb.Categories_category_name) LIKE UCASE('%{category}%')) WHEN ('{id_user}' != '' AND '{category}' = '' ) THEN (r.Users_id_user = '{id_user}') WHEN ('{id_user}' = '' AND '{category}' != '' ) THEN (UCASE(cb.Categories_category_name) LIKE UCASE('%{category}%')) WHEN ('{id_user}' = '' AND '{category}' = '' ) THEN 1 END"
               cur = db.connection.cursor()
               cur.execute(query)
               column_names = [i[0] for i in cur.description]
               res = [dict(zip(column_names, entry)) for entry in cur.fetchall()]
            ...
            query=f"SELECT  AVG(r.grade) AS avg_grade FROM Review r INNER JOIN Books b ON b.ISBN = r.Books_ISBN INNER JOIN Categories_Books cb ON cb.Books_ISBN = b.ISBN INNER JOIN Schools s ON s.id_school = b.Schools_id_school  WHERE s.Operators_id_operators = '{session['id']}' AND r.approved=1" 
            cur = db.connection.cursor()
            cur.execute(query)
            column_names = [i[0] for i in cur.description]
            res = [dict(zip(column_names, entry)) for entry in cur.fetchall()]
...
def add_school():
    ...
    if 'loggedin' in session and session['role']=='administrator':
        if(request.method == "POST" and form.validate_on_submit()):
           ...
           try:
               query=f"INSERT into Schools (name,address,city,phone_number,email,director_name,Administrator_id_admin,Operators_id_operators) VALUES ('{name}','{address}','{city}','{phone_number}','{email}','{director_name}','{session['id']}','{id_operator}')"
               cur = db.connection.cursor()
               cur.execute(query)
               db.connection.commit()
               cur.close()
